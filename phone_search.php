...
require_once('lib/phone_init.php');

show_template('phone_header');
...
switch ($action) { 
	case 'quick_search':
		...
		if (strlen($_REQUEST['search_string']) < 1) { 
			...
			show_template('show_phone_search');
			break;
		}
	case 'search':
	...
	case 'save_as_track':
		...
		show_confirmation("Search Saved","Your Search has been saved as a track in $playlist->name",conf('web_path') . "/phone_search.php");
	break;
	default:
		show_template('show_phone_search');
	break;
